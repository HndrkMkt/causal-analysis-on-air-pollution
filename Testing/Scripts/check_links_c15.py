                ...
                try:
                    ...
                    if counts[i[1]] < limit:
                        ...
                        for test in tests_to_evaluate:
                            ...
                            parent_links = pcmci.return_significant_parents(pq_matrix=q_matrix,
                                                                val_matrix=results['val_matrix'],
                                                                alpha_level=a)


                            valid_parents = list(parent_links['parents'].values())
                            ...
                            results_df = pd.DataFrame(test_results,
                                                                columns=['representation', 'complexity', 'sample_size', 'max_lag','test','alpha','valid_links_at_alpha',
                                                                           'learning_time'])
                            results_df.to_csv(
                                            '../Results/' + 'results_check_links_' + str(i[0]) + '_' + str(test[0]) + '.csv',
                                            index=False)

                            pickle_out = open(str(test[0]) + "_" + str(i[0]) + ".pickle", "wb")
                            pickle.dump(parent_links, pickle_out)
                            pickle_out.close()
...
networks = generate_DF(complexity=[15],sample_sizes=[1000])

print(str(len(networks)) + ' dataframes created ')

test(dataframes=networks,max_lags=[4],alpha=[0.05],tests=['RCOT','GPDC'],limit = 1)
